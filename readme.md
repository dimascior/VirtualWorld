...
> Screenshot above is generated by CI from a live keystroke session driven via
> `POST /input`. Each character cell = 2 vertically-stacked pixels via Unicode
> half-block `U+2580` (▀). The SVG is rebuilt and committed on every push to `main`.
